...
def recv(sock):
    ...
    while True:
        ...
        if (pkt.seq_num == ack_num + 1) & (pkt.chk_sum == pkt.compute_checksum()): # reply ACK IF the received packet is not corrupteed and has right sequence number
...
def check(): 
    ...
    true_pkt_buffer = _collect_pkt(
        sent_file_name)  # Read the file that is sent to this receiver and store data in a buffer
